...
function send_login_code() {
  ...
  if (!$user) wp_send_json_error(['message' => __('Unable to find a user with that email address.', 'client-power-tools')]);
  ...
  ob_start();
    ?>
      <p><?php _e('Here is the login code you requested', 'client-power-tools'); ?>: </p>
...
function check_login_code() {
  ...
  if (!$user) wp_send_json_error(['message' => __('User missing.', 'client-power-tools')]);
  ...
  if (!$code) {
    ...
    if ($tries < 3) {
      ...
      wp_send_json_error(['message' => __('Too many tries. Login failed.', 'client-power-tools')]);
    }
  }
